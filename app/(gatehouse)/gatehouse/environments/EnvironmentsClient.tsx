...
import { createEnvironmentAction } from "@/lib/gatehouse/server-actions";
...
export function EnvironmentsClient({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/gatehouse/environments");
      const list = await res.json();
      const flagsRes = await fetch("/api/gatehouse/flags");
      const flags = await flagsRes.json();
      const counts: Record<string, number> = {};
      for (const flag of flags) {
        const detail = await fetch(`/api/gatehouse/flags/${encodeURIComponent(flag.key)}`).then((r) => r.json());
