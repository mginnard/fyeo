  ...
  return (
    <section className="space-y-6">
      ...
      <p className="text-gray-700 dark:text-gray-300 text-sm">
        Add fyeo to your existing Next.js app in a few steps.
      </p>
      ...
      <div className="space-y-1 text-gray-700 dark:text-gray-300 text-sm">
        ...
        <ul className="list-disc list-inside space-y-0.5 text-gray-600 dark:text-gray-400">
          ...
          <li>Tailwind CSS (the fyeo dashboard UI uses Tailwind)</li>
        </ul>
      </div>

      <ol className="list-decimal list-inside space-y-4 text-gray-700 dark:text-gray-300 text-sm">
        <li>
          <strong className="text-gray-900 dark:text-white">Get the fyeo code</strong> — Clone or download the fyeo repository. You will copy three pieces from it into your app.
        </li>
        <li>
        ...
        <li>
          <strong className="text-gray-900 dark:text-white">Install dependencies</strong> — fyeo needs <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">better-sqlite3</code>, <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">framer-motion</code>, and <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">@types/better-sqlite3</code>. In your project run:
          <CodeBlock lang="bash">
{`npm install better-sqlite3 framer-motion && npm install -D @types/better-sqlite3`}
          </CodeBlock>
        </li>
        <li>
          <strong className="text-gray-900 dark:text-white">Configure environment</strong> — Copy <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">.env.example</code> from fyeo into your project as <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">.env.local</code>, or create <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">.env.local</code> and set:
          <ul className="mt-2 list-disc list-inside space-y-0.5 text-gray-600 dark:text-gray-400">
            <li><code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">FYEO_SECRET=your-secret-here</code> (required for protecting the admin API)</li>
            <li>Optionally <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">FYEO_ENV=development</code> (default is <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">development</code>)</li>
          </ul>
...
export function CreatingAndUsingFlagsSection() {
  return (
    <section className="space-y-4">
      ...
      <p className="text-gray-700 dark:text-gray-300 text-sm">
        In your app you evaluate flags by key and optional user context. fyeo uses the current <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">FYEO_ENV</code> to pick the right environment config.
      </p>
    </section>
  );
}
...
const showBanner = flags["promo-banner"] === true;`}
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm mb-2">
            Use <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">useFlag</code> inside components wrapped by <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">FyeoProvider</code>. The root layout should fetch flags and pass them in.
          </p>
          <CodeBlock lang="tsx">
{`// In app/layout.tsx: wrap with FyeoProvider and pass flags from getAllFlags()
import { getAllFlags } from "@/lib/fyeo/sdk";
import { FyeoProvider } from "@/lib/fyeo/client";
...
export async function middleware(request: Request) {
  ...
  return NextResponse.next();
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm mb-2">
            For custom default context or shared config, use <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">fyeo.config.ts</code> and call <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">createFyeo()</code>. Then use <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">getFlag</code> / <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">getAllFlags</code> from <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 font-mono text-xs">@/lib/fyeo</code> as above.
          </p>
          <CodeBlock lang="ts">
{`// fyeo.config.ts
