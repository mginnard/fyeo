...
import { evaluateFlag, evaluateAllFlags } from "@/lib/fyeo/evaluator";
import type { UserContext } from "@/lib/fyeo/types";

const DB_PATH = process.env.FYEO_DB_PATH ?? ".fyeo/flags.db";
const SECRET = process.env.FYEO_SECRET;

function auth(request: NextRequest): boolean {
  if (!SECRET) return true;
  const header = request.headers.get("x-fyeo-secret");
...
export async function POST(
  ...
  if (route[0] === "evaluate" && route.length === 1) {
    ...
    const env = process.env.FYEO_ENV ?? "development";
    const ctx = context ?? {};
